...
void main(void) {
	...
	fPosition = vPosition; // Pass model-space position to fragment shader
}
`;
...
void main(void) {
	...
	// Chance to simply discard the fragment if it's far from the center
	vec2 normalizedPos = fPosition.xz;
	vec2 center = vec2(1.0, 1.0);
	float distance = pow(length(normalizedPos - center) / 100.0, 2.0);
	float maxDistance = 70.0;
	float probability = distance / maxDistance;
	float noiseValue = psrddnoise(normalizedPos, vec2(0.0), 0.0, gradient, dg);
	...
	gl_FragColor = vec4(vec3(0.0, 0.2, 0.6) + causticColor, 1.0);
}
`;
